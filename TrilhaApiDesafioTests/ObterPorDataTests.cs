...
{
    public class ObterPorDataTests : IClassFixture<IntegrationTestAppFactory<Program>>
    {
        private readonly IntegrationTestAppFactory<Program> _factory;

        public ObterPorDataTests(IntegrationTestAppFactory<Program> factory)
        {
            _factory = factory;
        }

        [Fact]
        public async Task ObterPorData_TarefasExistem_DeveRetornarOkComTarefa()
        {
            // Arrange
            ...
            // Act
            var response = await httpClient.GetAsync($"/Tarefa/ObterPorData?data={tarefaEsperada.Data:yyyy-MM-dd}");
